...
{
    public class EfUserOperationClaimDal : EfEntityRepositoryBase<UserOperationClaim, EmlakPortalDb>, IUserOperationClaimDal
    {
        public List<UserOperationClaimDto> GetDtos(Expression<Func<UserOperationClaimDto, bool>> filter = null)
        {
            using (EmlakPortalDb context = new EmlakPortalDb())
            {
